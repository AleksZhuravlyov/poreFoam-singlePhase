...
#   include "createTime.H"
  ...
  #include "createCVs.H"
	...
	double porVol = gSum(mesh.V().field()*porosity.internalField());
	...
	std::valarray<std::valarray<double> > ditribLogU = distribution(log10(max(1.0e-16,(mag(clip.internalField()*U.internalField())/porosity.internalField()/VDarcy[iDir])->field()) ), mesh.V()*porosity.internalField());
	...
	std::valarray<std::valarray<double> > ditribUx = distribution(clip.internalField()*U.internalField().field().component(vector::X)/porosity.internalField()/VDarcy[iDir], mesh.V()*porosity.internalField());
	std::valarray<std::valarray<double> > ditribUy = distribution(clip.internalField()*U.internalField().field().component(vector::Y)/porosity.internalField()/VDarcy[iDir], mesh.V()*porosity.internalField());
	std::valarray<std::valarray<double> > ditribUz = distribution(clip.internalField()*U.internalField().field().component(vector::Z)/porosity.internalField()/VDarcy[iDir], mesh.V()*porosity.internalField());
